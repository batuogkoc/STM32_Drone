...
extern uint32_t this_time;
uint32_t inputCapture;	
extern uint32_t ch[8];
extern uint32_t ch_count;
/* USER CODE END 0 */
...
void TIM1_CC_IRQHandler(void)
{
  /* USER CODE BEGIN TIM1_CC_IRQn 0 */
	inputCapture = __HAL_TIM_GET_COUNTER(&htim1);
  ...
  /* USER CODE BEGIN TIM1_CC_IRQn 1 */
	...
	if(ch_count<8){
		...
		//without filter
		ch[ch_count] = inputCapture;
		
		
		//with filter
		/*if(abs((int)ch[ch_count] - (int)inputCapture) > 5){
			return;
		}
		else{
			ch[ch_count] = inputCapture;
